...
async def send_product(client, bot, product, CANAL_ID, state):
    ...
    if (not product_id or not image_url or not message or not store or not discount):
        return
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            props = {
                'sent_to_group_PREMIUM' if state == PREMIUM else 'sent_to_group_FREE': True
            }
        ...
        if attempt < max_retries:
            ...
            log_message("Se han agotado los intentos para enviar el producto.")